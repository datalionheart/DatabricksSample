...
`debezium CDC connector for oracle`  `strimzi kafka connect cluster` `azure eventhub`

`azure kubernetes services` `oracle 19c CDB/PDB` `oracle logminer`

# Objective

Debezium Oracle Connector on Strimzi Kafka Connect Cluster Integration w/ Azure Event Hub
...
```bash
cat << EOF > eventhub-oracle-credentials.yaml
apiVersion: v1
kind: Secret
metadata:
  name: eventhub-oracle-secret
type: Opaque
stringData:
...
kubectl apply -f eventhub-oracle-credentials.yaml -n ns4debezium
```
...
- apiGroups: [""]
  resources: ["secrets"]
  resourceNames: ["debezium-secret-oracle", "eventhub-oracle-secret"]
  verbs: ["get"]
EOF
...
spec:
  ...
  authentication:
    ...
    passwordSecret:
      secretName: eventhub-oracle-secret
...
```bash
cat << EOF > oracle-eventhub-connector.yaml
apiVersion: kafka.strimzi.io/v1beta2
kind: KafkaConnector
metadata:
...
kubectl apply -f oracle-eventhub-connector.yaml -n ns4debezium
```

### Check CDC Topic Status

![](./strimzi-debezium-eventhub-picture/status001-oracle.png)

![](./strimzi-debezium-eventhub-picture/status002-oracle.png)

![](./strimzi-debezium-eventhub-picture/status003-oracle.png)

![](./strimzi-debezium-eventhub-picture/status004-oracle.png)
